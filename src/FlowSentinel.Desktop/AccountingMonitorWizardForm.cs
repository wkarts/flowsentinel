...
internal sealed class AccountingMonitorWizardForm : Form
{
    ...
    {
        _name.Text = "Monitoramento da conferência contábil";
        ...
        _worksheet.PlaceholderText = "Ex.: CONF CONT 2026";
        ...
        _statusChanges.Text = "Notificar mudança em qualquer célula de situação por empresa e período";
        _currentStatusChanges.Text = "Notificar mudança da situação atual do cliente (último período preenchido)";
        _collaboratorChanges.Text = "Notificar mudança de colaborador responsável";
        _countChanges.Text = "Notificar alteração das quantidades por situação";
    ...
    private void BuildLayout()
    {
        ...
        var intro = new Label
        {
            ...
            Text = "Este assistente configura toda a planilha de uma só vez. Cada linha é uma empresa e cada célula de período é monitorada individualmente; não é necessário criar uma automação por cliente."
        };
        ...
        _analysisSummary.Text = "Selecione a planilha e clique em 'Analisar estrutura' para conferir empresas, seções, períodos e avisos antes de criar.";
    ...
    private async Task AnalyzeStructureAsync()
    {
        try
        {
            ...
            _analysisSummary.Text = $"Estrutura reconhecida: {analysis.CompanyCount:N0} clientes, {analysis.SectionCount:N0} seções, " +
    ...
    private void CreateDefinition()
    {
        try
        {
            ...
            if (selectedChannels.Count > 0)
            {
                if (_statusChanges.Checked)
                {
                    actions.Add(BuildAction(
                        ...
                        "{{Company}} - {{Period}}",
                        "A situação de {{Company}} (código {{Code}}), seção {{Section}}, período {{Period}}, mudou de '{{previous.Status}}' para '{{Status}}'. Célula: {{CellAddress}}. Responsável: {{Collaborator}}."));
                }
                if (_currentStatusChanges.Checked)
                {
                    actions.Add(BuildAction(
                        ...
                        selectedChannels,
                        [Equal("__recordType", "Company"), Changed("CurrentStatus")],
                        "Situação atual alterada - {{Company}}",
                        "A situação atual de {{Company}} (código {{Code}}), seção {{Section}}, mudou de '{{previous.CurrentStatus}}' para '{{CurrentStatus}}'. Período atual: {{CurrentPeriod}}. Responsável: {{Collaborator}}."));
                }
                if (_collaboratorChanges.Checked)
                {
                    actions.Add(BuildAction(
                        ...
                        selectedChannels,
                        [Equal("__recordType", "Company"), Changed("Collaborator")],
                        "Mudança de responsável - {{Company}}",
                        "A empresa {{Company}} (código {{Code}}), seção {{Section}}, mudou do colaborador '{{previous.Collaborator}}' para '{{Collaborator}}'."));
                }
            ...
            Definition = new AutomationDefinition
            {
                ...
                Description = "Monitoramento administrativo de planilha contábil com múltiplas seções, empresas, períodos, cores e totais por situação.",
                Enabled = true,
    ...
    private DataSourceDefinition BuildSource()
    {
        var settings = new
        {
            ...
            designerFields = new[]
            {
                "__recordType", "Worksheet", "Year", "Section", "Regime", "CompanyKey", "Company",
                "Code", "Collaborator", "Period", "PeriodBase", "Status", "StatusDisplay", "StatusMeaning",
                "CurrentPeriod", "CurrentStatus", "CurrentStatusDisplay", "Metric", "Unit", "Count",
        ...
        IReadOnlyCollection<(ChannelConfiguration Configuration, string Recipient)> channels)
    {
        var action = BuildAction(
            ...
            "Destaque alterado - {{Company}} / {{Period}}",
            "A formatação de {{Company}} (código {{Code}}), seção {{Section}}, período {{Period}}, foi alterada. Cor: '{{previous.FillColor}}' → '{{FillColor}}'; destaque: '{{previous.IsHighlighted}}' → '{{IsHighlighted}}'. Célula: {{CellAddress}}.");
